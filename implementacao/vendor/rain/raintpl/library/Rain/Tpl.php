    ...
    protected function compileFile($templateName, $templateBasedir, $templateDirectory, $templateFilepath, $parsedTemplateFilepath) {
        ...
        if (flock($fp, LOCK_SH)) {
            ...
            if (!is_writable(static::$conf['cache_dir']))
                throw new Tpl\Exception('Cache directory ' . static::$conf['cache_dir'] . 'doesn\'t have write permission. Set write permission or set RAINTPL_CHECK_TEMPLATE_UPDATE to FALSE. More details on https://www.raintpl.com/Documentation/Documentation-for-PHP-developers/Configuration/');
