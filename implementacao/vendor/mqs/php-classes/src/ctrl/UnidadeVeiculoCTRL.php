 ...
 */
class UnidadeVeiculoCTRL extends ModelCTRL {
    
    private $unidadeVeiculoDAO;
    ...
    public function __construct($banco = NULL, $datas = NULL, $textos = NULL){
        ...
        $this->unidadeVeiculoDAO 	= new UnidadeVeiculoDAO($this->getBanco(), $this->getDatas(), $this->getTextos());
    }
    ...
    public function getListaRegistro(){
        ...
        $funcionalidade = $this->getAcessoriosCTRL()->retornaFuncionalidades(" AND tab.codigo = 'unidade_veiculo_manter' ");
        ...
        $page->setTpl("unidade_veiculo_manter", array(
            "pagina" => $pagina,
            "sessao" => $sessao,
    ...
    public function setRegistro($valores,$arquivos){
        ...
        $campos = array();
        $acao						        = $_POST['acao'];
        $campos['idUnidadeVeiculo']         = $_POST['idUnidadeVeiculo'];
        $campos['unidadeVeiculo']           = $_POST['unidadeVeiculo'];
        $campos['codigo']                   = $_POST['codigo'];
        $campos['observacoes']              = $_POST['observacoes'];
        $campos['idVeiculo']                = $_POST['idVeiculo'];
        $campos['idUnidade']                = $_POST['idUnidade'];
        ...
        try {
            switch($acao){
                case "excluir":
                    ...
                    $camposExcluir = array();
                    $camposExcluir['idUnidadeVeiculo']          = $campos['idUnidadeVeiculo'];
    ...
    function retornaLista($where, $lazy = FALSE){
        try {
            ...
            return $this->unidadeVeiculoDAO->retornaLista($where,$lazy);
        } catch (\Exception $e) {
            return array();
        }
    }
    ...
    function validacoes($campos,$tipo){
        ...
        try {
            ...
            if($tipo == "CADASTRAR"){
                ...
                if(!$erro){ $erro = $this->getValida()->validaVazio($campos['idUnidade']); if($erro){ $msgErro[] = "O campo UNIDADE deve ser preenchido."; } }
            }
            if(!$erro){
                $itens['tabela']	= "unidades_veiculos";
                $itens['where']		= " descricao = '".$this->textos->encodeToIso($campos['unidadeVeiculo'])."' ";
                if($tipo == "ALTERAR"){
                    $erro = $this->getValida()->validaVazio($campos['idUnidadeVeiculo']);
                    if($erro){ $msgErro[] = "Defina o registro para alteração."; }
                    $itens['where']		= " descricao = '".$this->textos->encodeToUtf8($campos['unidadeVeiculo'])."' AND ativo = 'true' AND id <> ".$campos['idUnidadeVeiculo']." ";
                }
    ...
    function inserir($campos){
        ...
        try {
            ...
            if(!$validacao['erro']){
                ...
                $novoId = $this->unidadeVeiculoDAO->inserir($campos);
                if($novoId == 0){
                    $validacao['erro']		= TRUE;
    ...
    function alterar($campos){
        ...
        try {
            // operação
            if(!$validacao['erro']){
                $this->unidadeVeiculoDAO->alterar($campos);
            }
