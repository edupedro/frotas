 ...
 */
class LocalCTRL extends ModelCTRL {
    ...
    public function __construct($banco = NULL, $datas = NULL, $textos = NULL){
        ...
        $this->localDAO 	= new LocalDAO($this->getBanco(), $this->getDatas(), $this->getTextos());
    }
    
    /**
     * Abrir página de local
     *
    ...
    public function getListaRegistro($paginaAtual){
        ...
        $funcionalidade = $this->getAcessoriosCTRL()->retornaFuncionalidades(" AND tab.codigo = 'local_cadastro' ");
        ...
        $page->setTpl("local_cadastro", array(
            "pagina" => $pagina,
            "sessao" => $sessao,
    ...
    function getRegistro($idRegistro = NULL){
        ...
        $page = new PageAdmin();
        $page->setTpl("local_cadastro_registro", array(
            "pagina" => $pagina,
            "sessao" => $sessao,
            "funcionalidade" => $funcionalidade[0],
            "isAdmin" => $isAdmin,
    ...
    public function setRegistro($valores,$arquivos){
        ...
        $campos = array();
        $acao						   = $valores['acao'];
        $campos['idLocal']             = $valores['idRegistro'];
        $campos['codigo']              = $valores['codigo'];
        $campos['local']               = $valores['local'];
        ...
        try {
            switch($acao){
                case "excluir":
                    ...
                    $camposExcluir = array();
                    $camposExcluir['idLocal']           = $campos['idLocal'];
     ...
     */
    function getMapaDeslocamento(){
        UsuarioCTRL::verifyLogin();
                
        $pagina = $this->carregarParametrosPagina();
        $sessao = $_SESSION[UsuarioCTRL::SESSION];
        $funcionalidade = $this->getAcessoriosCTRL()->retornaFuncionalidades(" AND tab.codigo = 'mapa_deslocamentos' ");
        $isAdmin = $this->isAdmin($sessao);
        
        $listaLocais = $this->retornaLista(" ORDER BY tab.descricao DESC ");
        
        $page = new PageAdmin([
            "header" => false,
            "footer" => false
        ]);
        $page->setTpl("mapa-deslocamento", array(
            "pagina" => $pagina,
            "sessao" => $sessao,
            "funcionalidade" => $funcionalidade[0],
            "isAdmin" => $isAdmin,
    ...
    function retornaLista($where, $lazy = FALSE){
        try {
            ...
            return $this->localDAO->retornaLista($where,$lazy);
        } catch (\Exception $e) {
            return array();
        }
    }
    ...
    function retornaQTD($where){
        try {
            return $this->localDAO->retornaQTD($where);
        } catch (\Exception $e) {
            return 0;
        }
    }
    ...
    function validacoes($campos,$tipo){
        ...
        try {
            ...
            $msgErro	= array();
            if($tipo == "CADASTRAR"){
                if(!$erro){ $erro = $this->getValida()->validaVazio($campos['local']); if($erro){ $msgErro[] = "O campo DESCRIÇÃO deve ser preenchido."; } }
            }
            if(!$erro){
                $itens['tabela']	= "locais";
                $itens['where']		= " descricao = '".$this->getTextos()->encodeToIso($campos['local'])."' ";
                if($tipo == "ALTERAR"){
                    $erro = $this->getValida()->validaVazio($campos['idLocal']);
                    if($erro){ $msgErro[] = "Defina o registro para alteração."; }
                    $itens['where']		= " descricao = '".$this->getTextos()->encodeToIso($campos['local'])."' AND ativo = 'true' AND id <> ".$campos['idLocal']." ";
                }
        ...
        try {
            ...
            if(!$validacao['erro']){
                ...
                if(!empty($campos['upload01']['name'])){
                    $this->getUpload()->setValores($campos['upload01'],ROOT . DS . "views/images/locais/", "local_");
                ...
                if(!empty($campos['upload02']['name'])){
                    $this->getUpload()->setValores($campos['upload02'],ROOT . DS . "views/images/locais/", "local_");
                    $retorno	= $this->getUpload()->salvar();
                    if($retorno['resultado']){
                ...
                if(!empty($campos['upload03']['name'])){
                    $this->getUpload()->setValores($campos['upload03'],ROOT . DS . "views/images/locais/", "local_");
                    $retorno	= $this->getUpload()->salvar();
                    if($retorno['resultado']){
                ...
                if(!empty($campos['upload04']['name'])){
                    $this->getUpload()->setValores($campos['upload04'],ROOT . DS . "views/images/locais/", "local_");
                    $retorno	= $this->getUpload()->salvar();
                    if($retorno['resultado']){
                ...
                $novoId = $this->localDAO->inserir($campos);
                if($novoId == 0){
                    $validacao['erro']		= TRUE;
    ...
    function alterar($campos){
        ...
        try {
            ...
            if(!$validacao['erro']){
                ...
                $user = $this->retornaID($campos['idLocal']);
                
                if(!empty($campos['upload01']['name'])){
                    if($user['imagem01'] != "default-local.png"){
                        unlink(ROOT . DS . "views/images/locais/".$user['imagem01']);
                    }
                    $this->getUpload()->setValores($campos['upload01'],ROOT . DS . "views/images/locais/", "local_");
                    $retorno	= $this->getUpload()->salvar();
                    if($retorno['resultado']){
                ...
                if(!empty($campos['upload02']['name'])){
                    if($user['imagem02'] != "default-local.png"){
                        unlink(ROOT . DS . "views/images/locais/".$user['imagem02']);
                    }
                    $this->getUpload()->setValores($campos['upload02'],ROOT . DS . "views/images/locais/", "local_");
                    $retorno	= $this->getUpload()->salvar();
                    if($retorno['resultado']){
                ...
                if(!empty($campos['upload03']['name'])){
                    if($user['imagem03'] != "default-local.png"){
                        unlink(ROOT . DS . "views/images/locais/".$user['imagem03']);
                    }
                    $this->getUpload()->setValores($campos['upload03'],ROOT . DS . "views/images/locais/", "local_");
                    $retorno	= $this->getUpload()->salvar();
                    if($retorno['resultado']){
                ...
                if(!empty($campos['upload04']['name'])){
                    if($user['imagem04'] != "default-local.png"){
                        unlink(ROOT . DS . "views/images/locais/".$user['imagem04']);
                    }
                    $this->getUpload()->setValores($campos['upload04'],ROOT . DS . "views/images/locais/", "local_");
                    $retorno	= $this->getUpload()->salvar();
                    if($retorno['resultado']){
