 ...
 */
class RotaLocalCTRL extends ModelCTRL {
    
    private $rotaLocalDAO;
    ...
    public function __construct($banco = NULL, $datas = NULL, $textos = NULL){
        ...
        $this->rotaLocalDAO 	= new RotaLocalDAO($this->getBanco(), $this->getDatas(), $this->getTextos());
    }
    ...
    public function getListaRegistro(){
        ...
        $funcionalidade = $this->getAcessoriosCTRL()->retornaFuncionalidades(" AND tab.codigo = 'rota_local_manter' ");
        ...
        $page->setTpl("rota_local_manter", array(
            "pagina" => $pagina,
            "sessao" => $sessao,
    ...
    public function setRegistro($valores,$arquivos){
        ...
        $campos = array();
        $acao						        = $_POST['acao'];
        $campos['idRotaLocal']              = $_POST['idRotaLocal'];
        $campos['rotaLocal']                = $_POST['rotaLocal'];
        $campos['ordem']                    = $_POST['ordem'];
        $campos['posicao']                  = $_POST['posicao'];
        $campos['idRota']                   = $_POST['idRota'];
        ...
        try {
            switch($acao){
                case "excluir":
                    ...
                    $camposExcluir = array();
                    $camposExcluir['idRotaLocal']               = $campos['idRotaLocal'];
    ...
    function retornaLista($where, $lazy = FALSE){
        try {
            ...
            return $this->rotaLocalDAO->retornaLista($where,$lazy);
        } catch (\Exception $e) {
            return array();
        }
    }
    ...
    function validacoes($campos,$tipo){
        ...
        try {
            ...
            $msgErro	= array();
            if($tipo == "CADASTRAR"){
                if(!$erro){ $erro = $this->getValida()->validaVazio($campos['idRota']); if($erro){ $msgErro[] = "O campo ROTA deve ser preenchido."; } }
                if(!$erro){ $erro = $this->getValida()->validaVazio($campos['idLocal']); if($erro){ $msgErro[] = "O campo LOCAL deve ser preenchido."; } }
            }
            if(!$erro){
                $itens['tabela']	= "rotas_locais";
                $itens['where']		= " descricao = '".$this->textos->encodeToIso($campos['rotaLocal'])."' ";
                if($tipo == "ALTERAR"){
                    $erro = $this->getValida()->validaVazio($campos['idRotaLocal']);
                    if($erro){ $msgErro[] = "Defina o registro para alteração."; }
                    $itens['where']		= " descricao = '".$this->textos->encodeToUtf8($campos['rotaLocal'])."' AND ativo = 'true' AND id <> ".$campos['idRotaLocal']." ";
                }
    ...
    function inserir($campos){
        ...
        try {
            ...
            if(!$validacao['erro']){
                ...
                $novoId = $this->rotaLocalDAO->inserir($campos);
                if($novoId == 0){
                    $validacao['erro']		= TRUE;
    ...
    function alterar($campos){
        ...
        try {
            // operação
            if(!$validacao['erro']){
                $this->rotaLocalDAO->alterar($campos);
            }
