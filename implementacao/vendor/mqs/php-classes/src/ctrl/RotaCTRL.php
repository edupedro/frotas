 ...
 */
class RotaCTRL extends ModelCTRL {
    ...
    public function __construct($banco = NULL, $datas = NULL, $textos = NULL){
        ...
        $this->rotaDAO 	= new RotaDAO($this->getBanco(), $this->getDatas(), $this->getTextos());
    }
    ...
    public function getListaRegistro($paginaAtual){
        ...
        $funcionalidade = $this->getAcessoriosCTRL()->retornaFuncionalidades(" AND tab.codigo = 'rota_cadastro' ");
        ...
        $page->setTpl("rota_cadastro", array(
            "pagina" => $pagina,
            "sessao" => $sessao,
    ...
    function getRegistro($idRegistro = NULL){
        ...
        $page = new PageAdmin();
        $page->setTpl("rota_cadastro_registro", array(
            "pagina" => $pagina,
            "sessao" => $sessao,
            "funcionalidade" => $funcionalidade[0],
            "isAdmin" => $isAdmin,
    ...
    public function setRegistro($valores, $arquivos){
        ...
        $campos = array();
        $acao						  = $valores['acao'];
        $campos['idRota']             = $valores['idRegistro'];
        $campos['rota']               = $valores['rota'];
        ...
        try {
            switch($acao){
                case "excluir":
                    ...
                    $camposExcluir = array();
                    $camposExcluir['idRota']              = $campos['idRota'];
    ...
    function retornaLista($where, $lazy = FALSE){
        try {
            ...
            return $this->rotaDAO->retornaLista($where,$lazy);
        } catch (\Exception $e) {
            return array();
        }
    }
    ...
    function retornaQTD($where){
        try {
            return $this->rotaDAO->retornaQTD($where);
        } catch (\Exception $e) {
            return 0;
        }
    }
    ...
    function validacoes($campos,$tipo){
        ...
        try {
            ...
            $msgErro	= array();
            if($tipo == "CADASTRAR"){
                if(!$erro){ $erro = $this->getValida()->validaVazio($campos['rota']); if($erro){ $msgErro[] = "O campo DESCRIÇÃO deve ser preenchido."; } }
                if(!$erro){ $erro = $this->getValida()->validaVazio($campos['codigo']); if($erro){ $msgErro[] = "O campo CÓDIGO deve ser preenchido."; } }
            }
            if(!$erro){
                $itens['tabela']	= "rotas";
                $itens['where']		= " descricao = '".$this->textos->encodeToIso($campos['rota'])."' AND codigo = '".$campos['codigo']."' ";
                if($tipo == "ALTERAR"){
                    $erro = $this->getValida()->validaVazio($campos['idRota']);
                    if($erro){ $msgErro[] = "Defina o registro para alteração."; }
                    $itens['where']		= " descricao = '".$this->textos->encodeToUtf8($campos['rota'])."' AND codigo = '".$campos['codigo']."' AND ativo = 'true' AND id <> ".$campos['idRota']." ";
                }
    ...
    function inserir($campos){
        ...
        try {
            ...
            if(!$validacao['erro']){
                ...
                $novoId = $this->rotaDAO->inserir($campos);
                if($novoId == 0){
                    $validacao['erro']		= TRUE;
