 ...
 */
class ProprietarioCTRL extends ModelCTRL {
    ...
    public function __construct($banco = NULL, $datas = NULL, $textos = NULL){
        ...
        $this->proprietarioDAO 	= new ProprietarioDAO($this->getBanco(), $this->getDatas(), $this->getTextos());
    }
    ...
    public function getListaRegistro($paginaAtual){
        ...
        $funcionalidade = $this->getAcessoriosCTRL()->retornaFuncionalidades(" AND tab.codigo = 'proprietario_cadastro' ");
        ...
        $page->setTpl("proprietario_cadastro", array(
            "pagina" => $pagina,
            "sessao" => $sessao,
    ...
    function getRegistro($idRegistro = NULL){
        ...
        $page = new PageAdmin();
        $page->setTpl("proprietario_cadastro_registro", array(
            "pagina" => $pagina,
            "sessao" => $sessao,
            "funcionalidade" => $funcionalidade[0],
            "isAdmin" => $isAdmin,
    ...
    public function setRegistro($valores,$arquivos){
        ...
        $campos = array();
        $acao						   = $valores['acao'];
        $campos['idProprietario']      = $valores['idRegistro'];
        $campos['proprietario']        = $valores['proprietario'];
        ...
        try {
            switch($acao){
                case "excluir":
                    ...
                    $camposExcluir = array();
                    $camposExcluir['idProprietario']    = $campos['idProprietario'];
    ...
    function retornaLista($where, $lazy = FALSE){
        try {
            ...
            return $this->proprietarioDAO->retornaLista($where,$lazy);
        } catch (\Exception $e) {
            return array();
        }
    }
    ...
    function retornaQTD($where){
        try {
            return $this->proprietarioDAO->retornaQTD($where);
        } catch (\Exception $e) {
            return 0;
        }
    }
    ...
    function validacoes($campos,$tipo){
        ...
        try {
            ...
            $msgErro	= array();
            if($tipo == "CADASTRAR"){
                if(!$erro){ $erro = $this->getValida()->validaVazio($campos['proprietario']); if($erro){ $msgErro[] = "O campo DESCRIÇÃO deve ser preenchido."; } }
            }
            if(!$erro){
                $itens['tabela']	= "proprietarios";
                $itens['where']		= " descricao = '".$this->getTextos()->encodeToIso($campos['proprietario'])."' ";
                if($tipo == "ALTERAR"){
                    $erro = $this->getValida()->validaVazio($campos['idProprietario']);
                    if($erro){ $msgErro[] = "Defina o registro para alteração."; }
                    $itens['where']		= " descricao = '".$this->getTextos()->encodeToIso($campos['proprietario'])."' AND ativo = 'true' AND id <> ".$campos['idProprietario']." ";
                }
        ...
        try {
            ...
            if(!$validacao['erro']){
                ...
                if(!empty($campos['uploadAvatar']['name'])){
                    $this->getUpload()->setValores($campos['uploadAvatar'],ROOT . DS . "views/images/proprietarios/", "proprietario_");
                ...
                if(!empty($campos['uploadCapa']['name'])){
                    $this->getUpload()->setValores($campos['uploadCapa'],ROOT . DS . "views/images/proprietarios/", "proprietario_");
                    $retorno	= $this->getUpload()->salvar();
                    if($retorno['resultado']){
                ...
                $novoId = $this->proprietarioDAO->inserir($campos);
                if($novoId == 0){
                    $validacao['erro']		= TRUE;
    ...
    function alterar($campos){
        ...
        try {
            ...
            if(!$validacao['erro']){
                ...
                $user = $this->retornaID($campos['idProprietario']);
                
                if(!empty($campos['uploadAvatar']['name'])){
                    if($user['imagemAvatar'] != "default-avatar.png"){
                        unlink(ROOT . DS . "views/images/proprietarios/".$user['imagemAvatar']);
                    }
                    $this->getUpload()->setValores($campos['uploadAvatar'],ROOT . DS . "views/images/proprietarios/", "proprietario_");
                    $retorno	= $this->getUpload()->salvar();
                    if($retorno['resultado']){
                ...
                $this->proprietarioDAO->alterar($campos);
            }
